#!/bin/sh
# Screenshot capture for visual review; only callrecorder_it/.test-runtime are used.
set -eu
...
CALL_RECORDER_ADMIN_ENABLED=true CALL_RECORDER_SESSION_SECRET=test-secret-key-1234567890 $compose up -d --no-deps --force-recreate backend >/dev/null
for n in $(seq 1 40); do curl -fsS http://127.0.0.1:18080/healthz >/dev/null && break; sleep 1; done
/usr/bin/python3 "$root/tests/screenshots.py"
