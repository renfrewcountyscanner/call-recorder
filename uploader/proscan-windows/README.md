...
- Successfully processed source fingerprints are retained across restarts. Configure `completed_directory` to copy confirmed audio there while leaving each original recording untouched. `delete_uploaded_files` is an opt-in fallback when no completed directory is configured.
