...
`import_directory.py` recursively scans `/logger-import` and is intended to run from the included systemd timer every 15 minutes. It imports `.mp3`, `.wav`, and `.m4a` files through the legacy API, removes audio and matching `.json` sidecars only after a successful upload, and removes empty subdirectories. Files younger than two minutes are left alone so a copy can finish; failed files remain for retry and are logged.

Configure `/etc/call-recorder-import.env` with `DESTINATION_URL`, `UPLOAD_ID`, and `UPLOAD_KEY`. For files without sidecar metadata, set `SYSTEM_NAME` and `RECEIVER_ID`, or place files below a system-named subdirectory. Install the script and units under `/usr/local/lib/call-recorder` and `/etc/systemd/system`, create `/logger-import`, then run `systemctl enable --now call-recorder-import.timer`. Keep credentials outside Git.
